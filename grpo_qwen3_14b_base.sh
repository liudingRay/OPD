#!/bin/bash
# Qwen3-14B GRPO runtime launcher. Use submit_grpo_qwen3_14b_2node_4xa100.sbatch
# on Leonardo; that wrapper creates the shared two-node Ray cluster.
set -exo pipefail
...
export PROJECT_NAME=${PROJECT_NAME:-Qwen3-14B-GRPO}
...
export MINI_BATCH_SIZE=${MINI_BATCH_SIZE:-16} # Conservative 14B starting point; increase after a stable run.
...
export N_RESPONSES=${N_RESPONSES:-4} # Four samples per prompt is the 14B A100 pilot default.
...
export MODEL_DTYPE=${MODEL_DTYPE:-bfloat16} # 14B needs BF16 to leave room for rollout KV cache.
...
TEST_DATASET=${TEST_DATASET:-["$TEST_DATA_DIR/AIME25/test.parquet", "$TEST_DATA_DIR/AMC23/test.parquet", "$TEST_DATA_DIR/AIME24/test.parquet"]}
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-model/Qwen3-14B-Base}
...
export REWARD_MODEL_PATH=${REWARD_MODEL_PATH:-$ACTOR_MODEL_PATH} # GRPO keeps reward_model disabled.
...
export PARALLEL_SIZE=${PARALLEL_SIZE:-2} # Two-way TP is a safer 14B starting point on 8 GPUs.
...
export OUTLINES_CACHE_DIR="${OUTLINES_CACHE_DIR:-$OPD_CACHE_ROOT/outlines/$(uuidgen)}"

mkdir -p "$TRITON_CACHE_DIR" "$TORCHINDUCTOR_CACHE_DIR" "$HF_DATASETS_CACHE" \
    "$HF_HUB_CACHE" "$OUTLINES_CACHE_DIR" "$OPD_CACHE_ROOT/verl-rlhf"
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.entropy_from_logits_with_chunking=${ACTOR_ENTROPY_FROM_LOGITS_WITH_CHUNKING:-False} \
    $KL_ARGS \
    ...
    actor_rollout_ref.rollout.gpu_memory_utilization=${ROLLOUT_GPU_MEMORY_UTILIZATION:-0.45} \
    ...
    trainer.nnodes=${TRAINER_NNODES:-2} \
    trainer.save_freq=${TRAINER_SAVE_FREQ:-40} \
